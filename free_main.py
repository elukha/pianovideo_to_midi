...
class App:
    def __init__(self):
        ...
        self.root = tk.Tk()
        # タイトルに無料版であることを明記
        self.root.title("Pianorole to MIDI v1.0 (無料版)")
        ...
        self.current_process = None # 外部プロセス管理用
    ...
    def video_to_image_thread_logic(self):
        try:
            ...
            # ========================================================
            # 【変更点】無料版制限: 最初の45秒間だけを対象とする
            # ========================================================
            limit_seconds = 45  # 制限したい秒数
            max_frames = int(limit_seconds * self.avg_fps) # 秒数からフレーム数を計算

            original_frames = frames
            
            # 元の長さが45秒より長い場合のみ、フレーム数を45秒分に制限する
            if frames > max_frames:
                frames = max_frames
                print(f"Free Version Limit: {frames} frames ({limit_seconds} seconds)")
            else:
                print(f"Video is shorter than {limit_seconds} seconds. Converting all {frames} frames.")
            ...
            cmd = [
                ffmpeg_exe, '-i', self.filename, '-vcodec', 'png',
                '-vframes', str(self.total_video_frames), # 指定フレーム数(45秒分)でストップ
                output_pattern, '-y'
            ]
            ...
            if ret_code == 0:
                self.root.after(0, self.update_progress_ui, 100, "変換完了(無料版制限あり)")
                self.root.after(0, lambda: messagebox.showinfo("infomation", "変換が正常に終了しました\n(無料版のため最初の45秒のみ変換されました)"))
            else:
                if ret_code != -9 and ret_code != 1: 
